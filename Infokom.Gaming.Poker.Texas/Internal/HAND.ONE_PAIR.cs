...
namespace Infokom.Gaming.Poker.Texas.Internal
{
	...
	internal static partial class HAND
	{
		public static class ONE_PAIR
		{
			...
			internal static bool TryDetect(Ranks S, Ranks D, Ranks C, Ranks H, out Rank ρ1, out Rank ρ2, out Rank ρ3, out Rank ρ4)
			{
				...
				if (R is not 0)
				{
					...
					R = (S | D | C | H);
					if ((R = R.Exclude(ρ1)) is not 0)
					{
						ρ2 = R.Upmost();
						if ((R = R.Exclude(ρ2)) is not 0)
						{
							ρ3 = R.Upmost();
							if ((R = R.Exclude(ρ3)) is not 0)
							{	
								ρ4 = R.Upmost();
								return true;
							}
